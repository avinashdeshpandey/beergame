    ...
    for user_role in range(0, len(cntfacility)): ## for each facility get/generate demand & EOQs
        ...
        if current_user_role == user_role:
            ...
            if stocks[user_role][isim] > 0:  #
                ...
                if int(runtime_lstOflsts_incoming_demand[user_role][isim]) > 0:
                    if runtime_lstOflsts_incoming_demand[user_role][isim] > stocks[user_role][isim]:  # some part will be fullfilled from stocks & some will be backlogged
                        # incoming_demand = stocks + backlogg
                        ship_qty_demands = stocks[user_role][isim]
                        # reduce stocks & backlogs QTYs
            ...
            if iORDERGENMODE == 'Naive':
                naive_eoq = int(runtime_lstOflsts_incoming_demand[user_role][isim]) * int(supplier_lead_time[user_role]) - stocks[user_role][isim] - np.max([lstoflsts_pendingorders_today_scheduled[user_role][isim]-backlog[user_role][isim],0])
                ...
                if backlog[user_role][isim] > 0:
                    generate_EOQ_to_order = int(runtime_lstOflsts_incoming_demand[user_role][isim]) * int(supplier_lead_time[user_role]) + np.max([backlog[user_role][isim] - stocks[user_role][isim], 0])
                ...
                if stocks[user_role][isim] > 0:
                    generate_EOQ_to_order = np.max([int(runtime_lstOflsts_incoming_demand[user_role][isim]) * int(supplier_lead_time[user_role]) - stocks[user_role][isim], 0])
